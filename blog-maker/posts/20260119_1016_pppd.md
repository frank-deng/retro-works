...
### Alpine Linux
	...
	apk add ppp-daemon iptables dnsmasq
...
将`sysctl -w net.ipv4.ip_forward=1`添加到`/opt/bootlocal.sh`中作为第一条命令，然后执行`sudo filetool.sh -b`命令保存修改。
...
将上述`iptables`命令添加到`/opt/bootlocal.sh`中，紧跟`sysctl`，然后执行`sudo filetool.sh -b`命令保存修改。
...
执行以下命令设置PPP服务器开机自启动，然后启动PPP服务器：

	sudo rc-update add pppd
	sudo rc-update add isp-service
	sudo rc-service pppd start
	sudo rc-service isp-service start
...
执行`sudo rc-service dnsmasq start`启动`dnsmasq`。
...
将上述`iptables`命令添加到`/opt/bootlocal.sh`中，然后执行`sudo filetool.sh -b`命令保存修改。
