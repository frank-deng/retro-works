...
安装`socat`后，新建`/usr/local/bin/dialin-login.sh`并设置权限为`500`或`700`，内容如下：
...
安装`socat`和`agetty`后，新建`/usr/local/bin/dialin-login.sh`并设置权限为`500`或`700`，内容如下：
...
新建`/etc/init.d/dialin-service`，内容如下：
	...
	command_args="TCP-LISTEN:23,reuseaddr,nodelay,fork EXEC:'sh /usr/local/bin/dialin_login.sh',pty,raw,echo=0"
