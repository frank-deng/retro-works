...
### Login script `dialin-login.sh`

Resume to login prompt after logging out. Also closes local echo, enables binary mode for TELNET.

	#!/bin/bash
	TERM='ansi'
	TTY=$(tty)
	TTY=${TTY#/dev/}
	echo -ne '\xFF\xFD\x22\xFF\xFB\x01\xFF\xFB\x00\xFF\xFD\x00'
	while true; do
...
After installing `socat`, copy login script `dialin-login.sh` to `/usr/local/bin/dialin-login.sh` and set the permission as `500` or `700`.
...
After installing `socat`, `agetty`, `bash`, copy login script `dialin-login.sh` to `/usr/local/bin/dialin-login.sh` and set the permission as `500` or `700`.
...
Recommended `.bashrc` snippet for automatically launch `screen` after login.

	if [[ -z $STY && -z $SSH_CLIENT && -z $SSH_CONNECTION && ! $(tty) =~ '^/dev/tty' ]]; then
		exec screen -rR  #Remove leading `exec` before using Kermit
	fi
	export ROWS=24
	export COLUMNS=80
	export TERM='ansi'
...
Kermit protocol can be used for transmitting files via serial interface. For Debian, Ubuntu, install `ckermit` package to transmit files via Xmodem/Ymodem/Zmodem protocol. OpenEuler need compile `ckermit` from source. **Shell launched by `screen` cannot use Kermit protocol**, or file transmission will always fail.
