...
After installing `socat`, create `/usr/local/bin/dialin-login.sh` and set the permission as `500` or `700`, file content is as following:
...
After installing `socat` and `agetty`, create `/usr/local/bin/dialin-login.sh` and set the permission as `500` or `700`, file content is as following:
