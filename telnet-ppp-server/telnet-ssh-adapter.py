...
async def service_main(reader,writer):
    ...
    try:
        ...
        channel.get_pty(__loginInfo.get('term','ansi'),
            __loginInfo.get('cols',80), __loginInfo.get('lines',24))
        channel.invoke_shell()
        channel.setblocking(0)
        while trans.is_active() and channel.active and (not channel.closed):
            try:
