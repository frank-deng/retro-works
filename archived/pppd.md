...
推荐在单独的TinyCore Linux虚拟机上部署PPP服务器。如需支持FTP则可考虑Alpine Linux。

**TinyCore Linux中，任何文件修改后都需要执行`sudo filetool.sh -b`命令保存修改，否则重启后修改会被还原！！！**
...
### Alpine Linux
	...
	apk add socat ppp-daemon iptables dnsmasq
...
TinyCore Linux需要在`/opt/.filetool.lst`中添加以下路径：

	etc/ppp
	usr/local/etc/ppp

然后执行`sudo filetool.sh -b`命令保存修改。
...
将`net.ipv4.ip_forward=1`添加到`/etc/sysctl.conf`中，之后重启或执行`sysctl -p`命令生效。

### TinyCore Linux

将`sysctl -w net.ipv4.ip_forward=1`添加到`/opt/bootlocal.sh`中作为第一条命令。
...
执行完上述`iptables`命令后，执行以下命令保存转发规则并设置`iptables`开机自启动：
...
添加文件`/etc/init.d/isp-service`，内容如下：

	#!/sbin/openrc-run
	name="ISP Service"
	command="/usr/bin/socat"
	command_args="TCP-LISTEN:2345,reuseaddr,nodelay,fork system:'/usr/sbin/pppd \"\$SOCAT_PTY\"',pty,stderr,setsid"
...
执行以下命令设置PPP服务器开机自启动：
	...
	sudo rc-update add pppd
	sudo rc-update add isp-service
...
执行`sudo rc-update add dnsmasq`将`dnsmasq`设为开机自启动。

### TinyCore Linux

在`/opt/bootlocal.sh`中添加以下命令实现`dnsmasq`开机自启动：

	while ! ifconfig -a|grep -q "inet addr:10.0.2.15"; do sleep 1; done;
	dnsmasq

在`/opt/.filetool.lst`中添加`etc/dnsmasq.conf`，然后执行`sudo filetool.sh -b`命令保存修改。
...
执行完上述`iptables`命令后，执行以下命令保存转发规则并生效：
...
将上述`iptables`命令添加到`/opt/bootlocal.sh`中即可。
